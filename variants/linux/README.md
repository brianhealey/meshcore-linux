...
Native Linux support for MeshCore, targeting Raspberry Pi (Zero, 3, 4, 5) and similar SBCs with an SX1262 LoRa radio attached over SPI. Uses PlatformIO's `native` platform with thin POSIX shims (`variants/linux/Arduino.h`, `LinuxFS.h`, `LinuxHal.h`) to run the same firmware codebase on Linux without modification to the core library.
